...
<div class="container mx-auto p-6">
    <div class="bg-white shadow rounded-lg p-6">
        <form action="{{ route('pembayaran_beban.store') }}" method="POST">
            @csrf
            ...
            <div class="mb-4">
                ...
                <select name="no_akun" class="w-full border rounded p-2 @error('no_akun') border-red-500 @enderror" required>
                    ...
                    @foreach($akun_beban as $akun)
                        <option value="{{ $akun->no_akun }}" {{ old('no_akun') == $akun->no_akun ? 'selected' : '' }}>
            ...
            <div class="flex gap-2">
                ...
                <a href="{{ route('pembayaran_beban.index') }}" class="bg-gray-500 text-white px-4 py-2 rounded hover:bg-gray-600">Batal</a>
            </div>
        </form>
    </div>
</div>
