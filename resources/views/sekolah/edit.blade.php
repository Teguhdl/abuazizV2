    ...
    <div class="bg-white shadow rounded-lg p-6">
        ...
        <form action="{{ route('sekolah.update', $sekolah->id) }}" method="POST" class="space-y-4">
            ...
            <div>
                ...
                <textarea name="alamat" class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-400">{{ old('alamat', $sekolah->alamat) }}</textarea>
            </div>
            ...
            <div>
                ...
                <select name="jenis" class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-400">
                    ...
                    <option value="SD" {{ old('jenis', $sekolah->jenis) == 'SD' ? 'selected' : '' }}>SD</option>
                    <option value="SMP" {{ old('jenis', $sekolah->jenis) == 'SMP' ? 'selected' : '' }}>SMP</option>
                    <option value="SMA" {{ old('jenis', $sekolah->jenis) == 'SMA' ? 'selected' : '' }}>SMA</option>
                </select>
            </div>
